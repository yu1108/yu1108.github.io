...
function findClosestColor(r, g, b, mode) {
  ...
  } else if (mode === 'blackWhiteYellow') {
    palette = blackWhiteYellowPalette;
  } else {
    palette = rgbPalette;
  }

  // 黑白黄模式特殊处理：优先检测黄色
  if (mode === 'blackWhiteYellow') {
    // 黄色检测：红色和绿色都高，蓝色低
    if (r > 180 && g > 180 && b < 100) {
      return blackWhiteYellowPalette[2]; // 黄色
    }
    // 否则根据亮度选择黑或白
    const luminance = 0.299 * r + 0.587 * g + 0.114 * b;
    return luminance < 128 ? blackWhiteYellowPalette[0] : blackWhiteYellowPalette[1];
  }
...
function decodeProcessedData(processedData, width, height, mode) {
  ...
  if (mode === 'sixColor') {
    ...
    const byteWidth = Math.ceil(width / 8);
    const blackWhiteData = processedData.slice(0, byteWidth * height);
    const yellowWhiteData = processedData.slice(byteWidth * height);
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        const byteIndex = y * byteWidth + Math.floor(x / 8);
        const bitIndex = 7 - (x % 8);
        const blackWhiteBit = (blackWhiteData[byteIndex] >> bitIndex) & 1;
        const yellowWhiteBit = (yellowWhiteData[byteIndex] >> bitIndex) & 1;
        const index = (y * width + x) * 4;
        if (!yellowWhiteBit) {
          // 黄色
          data[index] = 255;
...
function processImageData(imageData, mode) {
  ...
  if (mode === 'sixColor') {
    ...
    const byteWidth = Math.ceil(width / 8);
    const blackWhiteThreshold = 140;
    const yellowThreshold = 180;

    const blackWhiteData = new Uint8Array(height * byteWidth);
    const yellowWhiteData = new Uint8Array(height * byteWidth);
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        // 黄色检测：红色和绿色都高，蓝色低
        const yellowWhiteBit = (r > yellowThreshold && g > yellowThreshold && b < 100) ? 0 : 1;
        const yellowWhiteByteIndex = y * byteWidth + Math.floor(x / 8);
        const yellowWhiteBitIndex = 7 - (x % 8);
        if (yellowWhiteBit) {
          yellowWhiteData[yellowWhiteByteIndex] |= (0x01 << yellowWhiteBitIndex);
        } else {
          yellowWhiteData[yellowWhiteByteIndex] &= ~(0x01 << yellowWhiteBitIndex);
        }
      }
    ...
    processedData = new Uint8Array(blackWhiteData.length + yellowWhiteData.length);
    processedData.set(blackWhiteData, 0);
    processedData.set(yellowWhiteData, blackWhiteData.length);
  }
